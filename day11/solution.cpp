...
auto getNextOutputs(aoc::Interpreter &i, Colour c) -> std::optional<Output> {
...
auto computeIncrement(aoc::Coord facing, Direction d) -> aoc::Coord {
  switch (d) {
  case Direction::left:
...
auto runPaint(aoc::Interpreter &i, Colour start) {
  auto pos = aoc::Coord{};
  auto facing = aoc::Coord{0, 1};
  std::map<aoc::Coord, Colour> result{{pos, start}};
...
auto getColour(const std::map<aoc::Coord, Colour> &m, aoc::Coord c) {
...
TEST_CASE("day11") {
  ...
  {
    auto comp = aoc::Interpreter{prog, std::array<int64_t, 0>{}};
