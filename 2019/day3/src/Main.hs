...
data Coord = Coord Int Int deriving (Show, Eq, Generic)
...
data Segment = Segment Coord Int deriving Show

charToVec char = case char of
  'U' -> Coord 0 1
  'D' -> Coord 0 (-1)
  'L' -> Coord (-1) 0
  'R' -> Coord 1 0

tokenToSegment (dir:length) = Segment (charToVec dir) (read length)

parsePath = map tokenToSegment . splitOn ","

addToMap (start, step, map) (Segment direction dist) = case dist of
  0 -> (start, step, map)
  _ -> let next = start + direction in
       let nextStep = step + 1 in
       let newMap = Map.insertWith (\_ old -> old) next nextStep map in
       addToMap (next, nextStep, newMap) (Segment direction (dist - 1)) 

buildMap segments = let (_, _, map) = L.foldl' addToMap (Coord 0 0, 0, Map.empty) segments in map

folder map fn acc (Coord x y) distIn = case Map.lookup (Coord x y) map of
  Nothing -> acc
  Just dist -> let result = fn (Coord x y) dist distIn in case acc of
    Nothing -> Just result
    Just val -> Just $ min val result

minIntersection fn map = Map.foldlWithKey' (folder map fn) Nothing 

manhattan (Coord x y) _ _ = abs x + abs y
steps _ distA distB = distA + distB
...
main = do
  let m1 = buildMap $ parsePath path1
  let m2 = buildMap $ parsePath path2
  print $ minIntersection manhattan m1 m2
  print $ minIntersection steps m1 m2