...
auto constexpr input = R"(2-4 r: prrmspx
5-6 p: hpzplphxb
5-8 t: ttttbtttttc
1-6 k: kkkkkk
1-3 q: qqqq
...
std::istream &operator>>(std::istream &is, Line &l) {
  ...
  if (std::smatch m; std::regex_search(
          line, m, std::regex(R"(([0-9]+)-([0-9]+) ([a-zA-Z0-9]): (.*))"))) {
    l.lower = std::stoi(m[1]);
    l.upper = std::stoi(m[2]);
    l.letter = *m[3].first;
    l.pwd = m[4].str();
  }
...
TEST_CASE("day2") {
  ...
  auto const a = std::count_if(lines.begin(), lines.end(), [](Line const &l) {
    auto const count = std::count(l.pwd.begin(), l.pwd.end(), l.letter);
    return l.lower <= count && count <= l.upper;
  });

  auto const b = std::count_if(lines.begin(), lines.end(), [](Line const &l) {
    auto const lower = l.pwd[l.lower - 1] == l.letter;
