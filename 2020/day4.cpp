...
namespace {
auto constexpr test_input = R"(ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
auto constexpr input = R"(iyr:1928 cid:150 pid:476113241 eyr:2039 hcl:a5ac0f
ecl:#25f8d2
byr:2027 hgt:190
...
struct Passport {
  ...
  std::optional<std::string> get(const std::string &key) const {
    if (auto const it = fields.find(key); it != fields.cend())
      return it->second;
...
std::istream &operator>>(std::istream &is, Passport &p) {
  ...
  for (std::string line; std::getline(is, line);) {
    ...
    std::vector const items(std::istream_iterator<std::string>{ls},
                            std::istream_iterator<std::string>{});

    for (auto const &item : items)
      if (auto const separator = item.find(":"); separator != std::string::npos)
        p.fields.emplace(item.substr(0, separator), item.substr(separator + 1));
  }
...
TEST_CASE("day4") {
  ...
  std::vector const passports(std::istream_iterator<Passport>{is},
                              std::istream_iterator<Passport>{});

  auto const a =
      std::count_if(passports.cbegin(), passports.cend(), [](auto const &p) {
        std::array const required{"byr", "iyr", "eyr", "hgt",
                                  "hcl", "ecl", "pid"};

        return std::all_of(required.cbegin(), required.cend(),
                           [&](auto const &r) { return p.get(r).has_value(); });
      });
