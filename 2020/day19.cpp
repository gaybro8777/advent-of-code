...
namespace {
auto constexpr rules_input = R"(62: 110 112 | 92 68
85: 38 92 | 102 110
63: 92 6 | 110 23
82: 110 | 92
99: 110 122 | 92 54
...
auto constexpr messages_input = R"(abbabbbbaaabbbbabbbababaaabaabaabbaabbba
...
struct Rule {
  int index{};
  std::variant<SequenceRule, EitherRule, StringRule> rule;
};

std::vector<int> parse_sequence(std::string const &text) {
  std::istringstream internal{text};
  return std::vector(std::istream_iterator<int>{internal},
...
std::istream &operator>>(std::istream &is, Rule &rule) {
  ...
  if (std::smatch m; std::regex_match(
          line, m, std::regex{R"regex(([0-9]+): ([0-9 ]+))regex"})) {
    rule.index = std::stoi(m[1]);
    rule.rule = SequenceRule{parse_sequence(m[2])};
    return is;
  }

  if (std::smatch m; std::regex_match(
          line, m,
          std::regex{R"regex(([0-9]+): ([0-9 ]+) \| ([0-9 ]+))regex"})) {
    rule.index = std::stoi(m[1]);
    rule.rule = EitherRule{parse_sequence(m[2]), parse_sequence(m[3])};
    return is;
  }

  if (std::smatch m; std::regex_match(
          line, m, std::regex{R"regex(([0-9]+): "([a-z]+)")regex"})) {
    rule.index = std::stoi(m[1]);
    rule.rule = StringRule{m[2]};
    return is;
  }
...
std::string build_regex(std::map<int, Rule> const &rules,
  ...
  for (auto const &i : indices)
    result.append(build_regex(rules, i));

  return result;
}

std::string build_regex(std::map<int, Rule> const &rules, int index) {
  auto const rule = rules.find(index);
  ...
  return std::visit(Overloaded{
                        ...
                        [&](EitherRule const &rule) {
                          return "(?:" + build_regex(rules, rule.first) + "|" +
                                 build_regex(rules, rule.second) + ")";
                        },
                        [](StringRule const &rule) { return rule.text; },
                    },
                    rule->second.rule);
}
} // namespace
...
TEST_CASE("day19") {
  auto const lines = [] {
    std::istringstream is{rules_input};
    return std::vector(std::istream_iterator<Rule>{is},
                       std::istream_iterator<Rule>{});
  }();

  auto const rules = [&] {
    std::map<int, Rule> result;

    for (auto const &line : lines)
      result.emplace(line.index, line);

    return result;
  }();

  auto const messages = [] {
    std::istringstream is{messages_input};
    return std::vector(std::istream_iterator<std::string>{is},
                       std::istream_iterator<std::string>{});
  }();

  auto const a = [&] {
    std::regex const regex{build_regex(rules, 0)};
    return std::count_if(
        messages.cbegin(), messages.cend(),
        [&](auto const &message) { return std::regex_match(message, regex); });
  }();

  auto const b = [&] {
    auto const rule42 = build_regex(rules, 42);
    auto const rule31 = build_regex(rules, 31);

    std::regex const regex42{"^" + rule42};
    std::regex const regex31{"^" + rule31};

    return std::count_if(
        messages.cbegin(), messages.cend(), [&](auto const &message) {
          auto copy = message;
          std::smatch m;

          auto count42 = 0;
          for (; std::regex_search(copy, m, regex42);) {
            count42 += 1;
            copy = m.suffix();
          }

          auto count31 = 0;
          for (; std::regex_search(copy, m, regex31);) {
            count31 += 1;
            copy = m.suffix();
          }

          return copy.empty() && count31 > 0 && count42 > count31;
        });
  }();
}
